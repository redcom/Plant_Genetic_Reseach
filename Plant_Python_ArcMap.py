...
class User_Interface(object):
    ...
    def __init__(self):
        ...
        # Creates widgets for a label and space to input for image file
        self.label_image = tkinter.Label(self.root, text="Image File")
        self.label_image.grid(row=3, column=0, sticky=tkinter.E)
        self.entrytext_image = tkinter.StringVar()
        self.e_image = tkinter.Entry(
                     self.root,
                     width=80,
                     textvariable=self.entrytext_image)
        self.e_image.grid(row=3, column=1, columnspan=5)
    ...
    def fill_recent_entries(self, event=None):
        ...
        if os.path.exists(entry_file):
            ...
            image_file = user_inputs[3]
            planning_file = user_inputs[4]
            var_row1 = user_inputs[5]
            var_row2 = user_inputs[6]
            var_row4 = user_inputs[7]
            var_even_spacing = user_inputs[8]
            var_diff_spacing = user_inputs[9]
            spacing1 = user_inputs[10]
            spacing2 = user_inputs[11]
            spacing3 = user_inputs[12]
            spacing4 = user_inputs[13]
            spacing5 = user_inputs[14]
            plot_length = user_inputs[15]
            plot_width = user_inputs[16]
            ...
            self.e_image.insert(0, image_file)
...
def Create_Polygon_Layer(df, shapefile, layer_name, field):
    ...
    arcpy.ImportToolbox("C:\Users\William.Yingling\AppData\Roaming\\"
                        "ESRI\Desktop10.3\ArcToolbox\My Toolboxes\\"
...
def Parameters_Executed(housing_folder, inputs):
    ...
    parameters.append("Image Copied: " + inputs[3])
    parameters.append(" ")

    parameters.append("Planning File: " + inputs[4])
    parameters.append("Rows: " + inputs[5])
    parameters.append("Plot Length in ft: " + inputs[6])
    parameters.append("Plot Width in inches: " + inputs[7])
...
def main():
    ...
    image_from_user = user_inputs[3]
    planning_file = user_inputs[4]
    rows = Find_Rows(user_inputs[5:8])
    var_even_spacing = user_inputs[8]
    var_diff_spacing = user_inputs[9]
    spacing = [float(user_inputs[10]), float(user_inputs[11]),
               float(user_inputs[12]), float(user_inputs[13]),
               float(user_inputs[14])]
    plot_width = int(user_inputs[15])
    plot_length = int(user_inputs[16])
    ...
    # create copy of image that user gave and put into our workspace folder
    base_image = Create_File_Copy(image_from_user, arcpy.env.workspace)
